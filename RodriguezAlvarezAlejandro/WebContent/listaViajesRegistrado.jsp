...
<body>
	...
	<c:when test="${!listaViajes.isEmpty()}">
			<tr>
				<th> Ir al promotor </th>
				<th>Origen</th>
				<th>Destino</th>
				<th>Plazas libres</th>
				<th>Coste estimado</th>
				<th>Solicitar plaza</th>
			</tr>
		<c:forEach var="entry" items="${listaViajes}" varStatus="i">
			<tr id="item_${i.index}">
				<td><a href="verPromotor?id=${entry.id}">Ir al promotor</a></td>
				<td>${entry.departure.city}</td>
				<td>${entry.destination.city}</td>
				<td>${entry.availablePax}</td>
				<td>${entry.estimatedCost}</td>
				<td><a id = "solicitarPlaza" href="solicitarPlaza?id=${entry.id}">Solicitar plaza</a></td>
			</tr>
		</c:forEach>
		</c:when>
		<c:otherwise>
